...
class SettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: Colors.transparent, title: const Text('Configuracion', style: TextStyle(color: Colors.white)), leading: IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white), onPressed: () => Navigator.pop(context))),
      body: ListView(padding: const EdgeInsets.all(20), children: [
        ...
        _switchTile(Icons.notifications_outlined, 'Notificaciones', notifProvider.enabled, (v) => notifProvider.toggleNotifications(v)),
        _tile(Icons.language, 'Idioma', () => Navigator.push(context, MaterialPageRoute(builder: (_) => const LanguageScreen()))),
        const SizedBox(height: 20),
        _section('Soporte'),
        _tile(Icons.help_outline, 'Centro de ayuda', () => Navigator.push(context, MaterialPageRoute(builder: (_) => const HelpCenterScreen()))),
        _tile(Icons.mail_outline, 'Contactar soporte', () => _contactSupport(context)),
        _tile(Icons.info_outline, 'Acerca de READ', () => _about(context)),
        const SizedBox(height: 30),
        Container(height: 52, margin: const EdgeInsets.symmetric(horizontal: 20), child: ElevatedButton(onPressed: () async { await context.read<app.AuthProvider>().logout(); if (context.mounted) Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const LoginScreen()), (route) => false); }, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFE53935), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))), child: const Text('Cerrar sesion', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)))),
        const SizedBox(height: 40),
      ]),
    );
  }

  Widget _section(String t) => Padding(padding: const EdgeInsets.only(left: 8, bottom: 12), child: Text(t, style: const TextStyle(color: Color(0xFFD7A15D), fontSize: 14, fontWeight: FontWeight.bold, letterSpacing: 1)));
  ...
  void _contactSupport(BuildContext context) async {
    final url = Uri.parse('mailto:soporte@read-libros.web.app');
    try { await launchUrl(url); } catch (_) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Email: soporte@read-libros.web.app'), backgroundColor: Color(0xFFD7A15D))); }
  }
}
